...
def backup():
    ...
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table \
            --default-character-set=utf-8 --quick awesome > %s' 
...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        r = run('ls -p -1')
        files = [s[:-1] for s in RE_FILES.split(r) if s.startswith('www-') and 
                 s.endswith('/')]
...
def restore2local():
    ...
    files = [f for f in fs if f.startswith('backup-') and 
             f.endswith('.sql.tar.gz')]
    files.sort(cmp=lambda s1, s2: 1 if s1 < s2 else -1)
    if len(files) == 0:
    ...
    sqls = [
        ...
        'grant select, insert, update, delete on awesome.* to \'%s\'@\
        \'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
